...
_EDA_LIBRARY_PATH = Path(__file__).resolve().parent.parent / "assets" / "signex-jlcpcb-basic.json"


@router.get("/eda/library")
async def eda_library(user: CurrentUser) -> Response:
    """The hosted component library for the Signex web build: JLCPCB
    Basic/Preferred parts pre-mapped to Signex rows + generic schematic
    symbols. Generated by scripts/gen_signex_library.py (checked in);
    the wasm app fetches it once per session and mounts it read-only.
    Auth: session cookie only — the iframe is same-origin, so the
    browser attaches it automatically."""
    if not _EDA_LIBRARY_PATH.is_file():
        raise HTTPException(status_code=404, detail="Library bundle not built")
    return Response(
        content=_EDA_LIBRARY_PATH.read_bytes(),
        media_type="application/json",
        # Immutable-ish: refreshed only by redeploying the backend image.
